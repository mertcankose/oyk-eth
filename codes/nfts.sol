...
contract ERC1155 is IERC1155 {
   ...
   constructor(string memory baseURI_) {
    ...
    function uri(uint256 id) public view virtual returns (string memory) {
        ...
        return bytes(_baseURI).length > 0 ? string(abi.encodePacked(_baseURI, toString(id), ".json")) : "";
    }
    ...
    function setTokenURI(uint256 id, string memory newTokenURI) external onlyOwner {
    ...
    function batchMint(address to, uint256[] calldata ids, uint256[] calldata values, bytes calldata data) external onlyOwner {
        require(to != address(0), "to = 0 address");
        require(ids.length == values.length, "ids length != values length");
    ...
    function batchBurn(address from, uint256[] calldata ids, uint256[] calldata values) external onlyOwner {
        require(from != address(0), "from = 0 address");
        require(ids.length == values.length, "ids length != values length");
